...
if __name__ == "__main__":
    ...
    print(__doc__)
    print("***********************SIMULATION for Jester BEGINS**************************")
    start = time.clock()
    cd = os.getcwd()
    data_folder= cd+'/data/jester'
    output_folder = cd+'/output/'
    df = pd.read_excel(data_folder+'/jester-data-1.xls', header=None)
    df = df.drop([0], axis=1) # remove the first column due to the joke recommendation count
    df[df==99] = -10
    df= df.head(3000)
    df = np.array(df)
    df = (df+10)/4
    ...
    #ratings.dump('jester_ratings_new.dat')
    #sys.exit()
    ...
    for k in matrix_factors:
        ...
        train_mse.dump(output_folder+'train_mse_jester_factor_'+str(k)+'.dat')
        test_mse.dump(output_folder+'test_mse_jester_factor_'+str(k)+'.dat')
        test_hr.dump(output_folder+'test_hr_jester_factor_'+str(k)+'.dat')
        test_precision.dump(output_folder+'test_precision_jester_factor_'+str(k)+'.dat')
    
    print("Done for Jester!.")
