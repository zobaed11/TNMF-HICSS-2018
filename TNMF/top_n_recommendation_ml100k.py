...
if __name__ == "__main__":
    ...
    start = time.clock()
    cd = os.getcwd()
    data_folder = cd + '/data/ml-100k'
    output_folder = cd + '/output/'
    names = ['user_id', 'item_id', 'rating', 'timestamp']
    df = pd.read_csv(data_folder+'/u.data', sep='\t', names=names)
    ...
    for k in matrix_factors:
        ...
        train_mse.dump(output_folder + 'train_mse_ml-100k_factor_'+str(k)+'.dat')
        test_mse.dump(output_folder + 'test_mse_ml-100k_factor_'+str(k)+'.dat')
        test_hr.dump(output_folder + 'test_hr_ml-100k_factor_'+str(k)+'.dat')
        test_precision.dump(output_folder + 'test_precision_ml-100k_factor_'+str(k)+'.dat')
    
    print("Done for ml-100k!")
