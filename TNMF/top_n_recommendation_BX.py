...
def train_test_split(ratings):
    ...
    for user in range(ratings.shape[0]):
        #test_ratings = np.random.choice(ratings[user, :].nonzero()[0], size=size, replace=True)
        if (len(list(ratings[user, :].nonzero()[0])) > size):
            test_ratings = random.sample(list(ratings[user, :].nonzero()[0]), size)
...
if __name__ == "__main__":
    ...
    print(__doc__)
    print("***********************SIMULATION for Book Crossing BEGINS**************************")
    start = time.clock()
    cd = os.getcwd()
    data_folder= cd+'/data/bookX'
    output_folder = cd+'/output/'
    names = ['user_id', 'isbn', 'rating']
    df = np.load(data_folder+'/bookX_ratings.dat')
    ratings = df

    
    # interesting..dynamic row deletion    
#    s = ratings.shape[0]
#    c = 0
#    k = 0
#    for i in range(s):
#        if (sum(ratings[:,i]) < size):
#            c += 1
#            print("Total "+ str(c) + " row(s) deleted!")
...
#            s = ratings.shape[0] 
    ...
    for k in matrix_factors:
        ...
        train_mse.dump(output_folder+'train_mse_bookX_factor_'+str(k)+'.dat')
        test_mse.dump(output_folder+'test_mse_bookX_factor_'+str(k)+'.dat')
        test_hr.dump(output_folder+'test_hr_bookX_factor_'+str(k)+'.dat')
        test_precision.dump(output_folder+'test_precision_bookX_factor_'+str(k)+'.dat')
    
    print("Done for Book crossing!.")
