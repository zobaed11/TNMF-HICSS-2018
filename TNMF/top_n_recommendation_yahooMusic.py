...
if __name__ == "__main__":
    ...
    print(__doc__)
    print("***********************SIMULATION for Yahoo! music BEGINS**************************")
    start = time.clock()
    cd = os.getcwd()
    data_folder= cd+'/data/yahoo'
    output_folder = cd+'/output/'
    names = ['user_id', 'item_id', 'rating', 'timestamp']
    df = np.load(data_folder+'/yahoo_music_ratings.dat')
...
#            s = ratings.shape[0] 
    ...
    for k in matrix_factors:
        ...
        train_mse.dump(output_folder+'train_mse_yahooMusic_factor_'+str(k)+'.dat')
        test_mse.dump(output_folder+'test_mse_yahooMusic_factor_'+str(k)+'.dat')
        test_hr.dump(output_folder+'test_hr_yahooMusic_factor_'+str(k)+'.dat')
        test_precision.dump(output_folder+'test_precision_yahooMusic_factor_'+str(k)+'.dat')
    
    print("Done for Yahoo! music.")
