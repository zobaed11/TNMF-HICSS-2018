...
class ALS(ModelBase):
    ...
    def __init__(self, n_user, n_item, n_feature, reg=1e-2, converge=1e-10,
...
#                logger.debug("no ratings for item %d", j)
    ...
    def fit(self, ratings, n_iters=50):
        ...
        last_rmse = None
        with open('/home/nsrg/Downloads/recommend-master/examples/data/als_bookX.txt', 'w') as file_handler:
#            file_handler.write('***ALS_with Yahoo_movie_ratings***' + '\n\n\n')
    ...
    #            logger.debug("iteration %d...", iteration)
                ...
                go='iter:'+ str(iteration)+'     train RMSE:'+ str(train_rmse)+'\n'
                print(go)
                
#                file_handler.write(str(iteration)+str('\t')+str(train_rmse)+'\n' )
                file_handler.write(str(train_rmse)+'\n' )
                logger.info("iter: %d, train RMSE: %.6f", iteration, train_rmse)
    #            print("iter: %d, train RMSE: %.6f, Precision: %3f", iteration, train_rmse)
                # stop when converge
                if last_rmse and abs(train_rmse - last_rmse) < self.converge:
    #                logger.info('converges at iteration %d. stop.', iteration)
                    print('converges at iteration %d. stop.', iteration)
                    break
                else:
